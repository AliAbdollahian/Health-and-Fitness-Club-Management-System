...
public class Controller {
    ...
    private void registerNewMember() {
        ...
        JTextField heightField = new JTextField();  // New height field
        ...
        if (option == JOptionPane.OK_OPTION) {
            try {
                ...
                int timeGoal = Integer.parseInt(timeGoalField.getText());

                // Creating member object; ID is auto-generated by the database
                Member newMember = new Member(0, fullName, dateOfBirth, fitnessGoal, weightGoal, timeGoal);
                jdbcDatabaseOp.addMember(newMember);// Add member

                int memberId = jdbcDatabaseOp.getMemberByName(fullName).getMemberId();

                // Adding health metrics with the new member ID
                jdbcDatabaseOp.addHealthMetrics(memberId, weightGoal, height);

                // Show member ID to the user
                JOptionPane.showMessageDialog(null, "Registration successful! Your Member ID is: " + jdbcDatabaseOp.getMemberByName(fullName).getMemberId());
